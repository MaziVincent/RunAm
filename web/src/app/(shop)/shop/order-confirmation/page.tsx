				...
				<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
					<div className="space-y-4">
						{orders.map((order) => (
							<Card
								key={order.id}
								className="rounded-3xl border-border/60 shadow-sm">
								...
								<CardContent className="space-y-4 p-5">
									<div className="grid gap-4 sm:grid-cols-2">
										...
										<div>
											...
											<p className="mt-2 text-sm text-foreground">
												{order.riderName || "Waiting for assignment"}
											</p>
										</div>
									</div>
						...
						{orders.length === 0 && (
							<Card className="rounded-3xl border-border/60 shadow-sm">
								<CardContent className="p-8 text-center text-sm text-muted-foreground">
									No matching order summary was found. You can still review your
									latest activity from the orders page.
								</CardContent>
							</Card>
						)}
					</div>
					...
					<Card className="h-fit rounded-3xl border-border/60 shadow-sm lg:sticky lg:top-24">
						...
						<CardContent className="space-y-4 text-sm">
							...
							<p className="text-muted-foreground">
								If you placed a multi-vendor checkout, each vendor order will
								progress independently in tracking.
							</p>
						</CardContent>
					</Card>
				</div>
