			...
			<div className="container py-12">
				<div className="mx-auto max-w-xl rounded-3xl border border-border/60 bg-card p-10 text-center shadow-sm">
					...
					<p className="mt-3 text-sm text-muted-foreground">
						Add meals from one or more vendors, then return here to review
						...
						<Card
								...
								<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
									<div>
										...
										<p className="mt-1 text-sm text-muted-foreground">
											{vendorCart.items.length} item
											{vendorCart.items.length === 1 ? "" : "s"} •{" "}
											{formatCurrency(vendorCart.subtotal)}
										</p>
									</div>
							...
							<CardContent className="space-y-4 p-5">
								{vendorCart.items.map((item, index) => {
									const unitPrice =
										item.unitPrice + (item.variant?.priceAdjustment ?? 0);
									const optionsPrice = item.extras.reduce(
				...
				<Card className="h-fit rounded-3xl border-border/60 shadow-sm lg:sticky lg:top-24">
					...
					<CardContent className="space-y-4">
						...
						<div className="rounded-2xl bg-muted/50 p-4 text-sm text-muted-foreground">
							Delivery is calculated at checkout using the shortest route across
							all selected vendors and your drop-off location.
						</div>
